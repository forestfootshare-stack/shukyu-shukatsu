...
function Header({ onOpenLine }) {
  const { isAuthed, user, profile, unreadReactions, signOut } = useApp();
    ...
    <header className="header">
      <div className="header-in">
        ...
        <div className="header-actions">
          {isAuthed ? (
            <>
              <Link to="/interested" className="icon-btn" aria-label={`あなたに興味を持っている企業${unreadReactions ? `（未読${unreadReactions}件）` : ""}`}>
                <BellIcon />
                {unreadReactions > 0 && <span className="dot">{unreadReactions}</span>}
              </Link>
              <div className="menu-anchor desk" ref={menuRef}>
                ...
                {menu && (
                  <div className="user-menu" role="menu">
                    ...
                    <Link to="/interested" role="menuitem">
                      興味を持っている企業 {unreadReactions > 0 && <span className="tag tag-amber">{unreadReactions}</span>}
                    </Link>
                    <Link to="/consults" role="menuitem">相談した企業</Link>
            ...
            <>
              ...
              <Link to="/interested">
                ...
                <span className="row" style={{ gap: 8 }}>
                  {unreadReactions > 0 && <span className="tag tag-amber">{unreadReactions}</span>}
                  <ArrowIcon />
                </span>
              </Link>
              <Link to="/consults">相談した企業<ArrowIcon /></Link>
              <Link to="/favorites">気になる企業<ArrowIcon /></Link>
    ...
    <footer className="footer">
      ...
      <div className="wrap">
        <div className="footer-bottom">
          ...
          <span>© 2026 FOREST FOOTBALL Inc. <span className="ver">v7</span></span>
        </div>
      </div>
    </footer>
  );
...
function LineFab({ onOpen, hidden, onHide }) {
  ...
  // ファーストビューを通過してから出す。最初から被せない。
  useEffect(() => {
    const onScroll = () => setShown(window.scrollY > window.innerHeight * 0.62);
    onScroll();
  ...
  return (
    <div className={`line-fab${shown ? " in" : ""}`}>
