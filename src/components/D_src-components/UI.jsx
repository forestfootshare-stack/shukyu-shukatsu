...
/* ---------------- 未ログイン時の認証ゲートモーダル ----------------
   企業への相談と、プロフィール公開の導線で使い分けます。 */
export function AuthGateModal({ onClose, next, variant = "apply", companyName }) {
  const nav = useNavigate();
  const go = (path) => nav(`${path}?next=${encodeURIComponent(next)}`);

  const copy = variant === "consult"
    ? {
        title: "相談にはアカウントが必要です",
        lead: companyName
          ? `${companyName}について詳しく聞くには、プロフィールの登録が必要です。登録は無料で、完了後はこのページに戻ります。`
          : "キャリアアドバイザーへの相談には、プロフィールの登録が必要です。登録は無料です。",
        cta: "無料で新規登録",
        items: ["仕事内容や選考について詳しく聞ける", "自分に合っている企業か相談できる", "相談だけでもOK。企業の選考ではありません"],
      }
    : {
        title: "競技経験を登録して、企業に見つけてもらおう",
        lead: "プロフィール登録は無料です。競技歴や希望条件を登録しておくと、あなたに興味を持った企業がわかります。",
        cta: "無料でプロフィール登録",
        items: ["競技経験も含めてプロフィールを作れる", "あなたに興味を持った企業がわかる", "公開はいつでもオフにできる"],
      };
