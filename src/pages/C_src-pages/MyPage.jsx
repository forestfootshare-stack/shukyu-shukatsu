...
const MENU = [
  ...
  { to: "/interested", label: "あなたに興味を持っている企業", badge: "reactions" },
  { to: "/consults", label: "相談した企業", badge: "consults" },
  { to: "/favorites", label: "気になる企業", badge: "favs" },
...
export function MyNav() {
  const { unreadReactions, consults, favorites, signOut } = useApp();
  const nav = useNavigate();
  const counts = { reactions: unreadReactions, consults: consults.length, favs: favorites.length };
        ...
        <NavLink key={m.to} to={m.to} end className={({ isActive }) => (isActive ? "on" : "")}>
          ...
          <span className="row" style={{ gap: 7 }}>
            {m.badge && counts[m.badge] > 0 && (
              <span className={`tag ${m.badge === "reactions" ? "tag-amber" : ""}`}>{counts[m.badge]}</span>
            )}
            <ArrowIcon size={14} />
          </span>
        </NavLink>
...
export default function MyPage() {
  const { user, profile, profileDone, completion, myReactions, consults, favorites } = useApp();
  const unread = myReactions.filter((s) => !s.read);
        ...
        <div className="my-layout">
          ...
          <div style={{ display: "grid", gap: 24 }}>
            ...
            <div className="grid-3" style={{ gap: 14 }}>
              {[
                { n: myReactions.length, label: "興味を持っている企業", to: "/interested" },
                { n: consults.length, label: "相談した企業", to: "/consults" },
                { n: favorites.length, label: "気になる企業", to: "/favorites" },
              ].map((s) => (
            ...
            <div>
              <div className="row-between" style={{ marginBottom: 6 }}>
                <h2 style={{ fontSize: 18 }}>あなたに興味を持っている企業</h2>
                <Link to="/interested" className="link-blue" style={{ fontSize: 13 }}>すべて見る</Link>
              </div>
              <p className="muted" style={{ marginBottom: 14 }}>
                プロフィールを見て興味を持った企業です。知らなかった企業を知るきっかけにしてください。
              </p>

              {!profile.openToCompanies && (
                <div className="notice" style={{ marginBottom: 14 }}>
                  企業へのプロフィール公開が現在オフになっています。
                  <Link to="/settings" className="link-blue">アカウント設定</Link> から変更できます。
                </div>
              )}

              {myReactions.length === 0 ? (
                <Empty title="まだ企業からのリアクションはありません" lead="プロフィールを充実させると、企業の目に留まりやすくなります。" />
              ) : (
                <div style={{ display: "grid", gap: 10 }}>
                  {myReactions.slice(0, 2).map((r) => {
                    ...
                    return (
                      <Link key={r.id} to={`/interested/${r.id}`} className={`card card-link scout-card${r.read ? "" : " unread"}`}>
                        ...
                        <div style={{ minWidth: 0, flex: 1 }}>
                          ...
                          <div className="muted" style={{ margin: "3px 0 8px" }}>{c.industry} / {c.locations.join(" / ")}</div>
                          <div className="row-wrap">
                            {r.points.slice(0, 2).map((pt) => (
                              <span key={pt} className="tag tag-blue">{pt}</span>
                            ))}
                          </div>
                        </div>
                      </Link>
            ...
            <div>
              <div className="row-between" style={{ marginBottom: 14 }}>
                <h2 style={{ fontSize: 18 }}>相談した企業</h2>
                <Link to="/consults" className="link-blue" style={{ fontSize: 13 }}>すべて見る</Link>
              </div>
              {consults.length === 0 ? (
                <Empty
                  title="まだ相談した企業はありません"
                  lead="気になる企業が見つかったら、キャリアアドバイザーに詳しく聞いてみましょう。"
                  actionTo="/companies"
                  actionLabel="企業を探す"
                />
              ) : (
                <div style={{ display: "grid", gap: 10 }}>
                  {consults.slice(0, 3).map((a) => {
                    ...
                    return (
                      <Link key={a.companyId} to={`/companies/${a.companyId}`} className="card card-link scout-card">
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div className="muted">相談日 {a.date}</div>
                        </div>
